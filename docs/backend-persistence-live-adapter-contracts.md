...
Report approval and evidence packet notification exports are JSON handoff contracts generated in the frontend. They include route stage, recipients, due dates, status, evidence summaries, and source dependencies without sending email or Teams messages directly. This keeps notification delivery pluggable while preserving an auditable payload for future delivery adapters.
