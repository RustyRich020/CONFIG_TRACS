...
Delivery retry controls are persisted as `notification_delivery_retry` records for notification closure package, closure SLA package, Closure SLA response follow-up, Postgres cutover acknowledgement, Postgres cutover owner reminder, and Postgres cutover closure package notifications. A retry control links to the original `notification_delivery` record, captures source, subject, recipients, channel list, retry policy, attempt count, retry eligibility, retry due timestamp, rationale, original result, optional retry result, and audit evidence. Executed retries resubmit the original delivery payload with a retry-specific delivery id and retain the new `notification_delivery` record separately from the retry-control record. The Backend workspace derives retry queue aging metrics from these records, falling back to `createdAt + retryDelayMinutes` for older records that do not yet carry `retryDueAt`.
